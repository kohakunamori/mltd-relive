...
public final class Http2Stream {
    ...
    public boolean isLocallyInitiated() {
        return this.connection.client == ((this.f738id & 1) == 1);
    }
    ...
    public void writeHeaders(List<Header> list, boolean z) throws IOException {
        ...
        this.connection.writeSynReply(this.f738id, z3, list);
        if (z2) {
    ...
    public void close(ErrorCode errorCode) throws IOException {
        if (closeInternal(errorCode)) {
            this.connection.writeSynReset(this.f738id, errorCode);
        }
    }

    public void closeLater(ErrorCode errorCode) {
        if (closeInternal(errorCode)) {
            this.connection.writeSynResetLater(this.f738id, errorCode);
        }
    }
    ...
    private boolean closeInternal(ErrorCode errorCode) {
        synchronized (this) {
            ...
            notifyAll();
            this.connection.removeStream(this.f738id);
            return true;
        }
    }
                            ...
                            if (Http2Stream.this.headersQueue.isEmpty() || Http2Stream.this.headersListener == null) {
                                if (this.readBuffer.size() > 0) {
                                    ...
                                    if (errorCode == null && Http2Stream.this.unacknowledgedBytesRead >= Http2Stream.this.connection.okHttpSettings.getInitialWindowSize() / 2) {
                                        Http2Stream.this.connection.writeWindowUpdateLater(Http2Stream.this.f738id, Http2Stream.this.unacknowledgedBytesRead);
    ...
    final class FramingSink implements Sink {
        ...
        private void emitFrame(boolean z) throws IOException {
            ...
            Http2Stream.this.writeTimeout.enter();
            try {
                Http2Stream.this.connection.writeData(Http2Stream.this.f738id, z && jMin == this.sendBuffer.size(), this.sendBuffer, jMin);
            } finally {
                Http2Stream.this.writeTimeout.exitAndThrowIfTimedOut();
            }
        }

        @Override // extern.okio.Sink, java.io.Flushable
        ...
        @Override // extern.okio.Sink, java.io.Closeable, java.lang.AutoCloseable
        public void close() throws IOException {
            synchronized (Http2Stream.this) {
                if (this.closed) {
                    return;
                ...
                if (!Http2Stream.this.sink.finished) {
                    if (this.sendBuffer.size() > 0) {
                        ...
                        Http2Stream.this.connection.writeData(Http2Stream.this.f738id, true, null, 0L);
                    }
                }
                synchronized (Http2Stream.this) {
                    this.closed = true;
