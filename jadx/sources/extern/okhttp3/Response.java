...
public final class Response implements Closeable {
    ...
    public ResponseBody peekBody(long j) throws IOException {
        ...
        Buffer bufferM574clone = bufferedSourceSource.buffer().m574clone();
        if (bufferM574clone.size() > j) {
            Buffer buffer = new Buffer();
            buffer.write(bufferM574clone, j);
            bufferM574clone.clear();
            bufferM574clone = buffer;
        }
        return ResponseBody.create(this.body.contentType(), bufferM574clone.size(), bufferM574clone);
    }
