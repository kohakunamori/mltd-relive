...
from mltd.servers.asset_server import (  # noqa: E402
    bind_asset_handler,
    create_server,
)
...
class ProxyTransportTest(unittest.TestCase):
    def setUp(self):
        ...
        self.temp = tempfile.TemporaryDirectory()
        self.store = AssetStore(self.temp.name)
        self.asset_name = 'proxy-sample.unity3d'
        self.asset_body = b'proxy-asset-body' * 128
        asset_path = self.store.object_path('zh', 'android', self.asset_name)
        asset_path.write_bytes(self.asset_body)
        self.store.put_metadata(
            'zh', 'android', self.asset_name,
            status=200,
            size=len(self.asset_body),
            sha256=hashlib.sha256(self.asset_body).hexdigest(),
            headers={'Content-Type': 'application/octet-stream'},
        )
        bind_asset_handler(proxy.ProxyHTTPRequestHandler, self.store)
    ...
    def test_tls_proxy_asset_prefix_maps_to_local_store(self):
        # Keep the old prefix route as a compatibility fallback for configs
        # generated by v0.1.8/v0.1.9. New clients should use the public asset
        # Host route instead.
        conn = http.client.HTTPConnection('127.0.0.1', self.port, timeout=3)
        conn.request(
            'GET', f'/__mltd_assets/zh-android/{self.asset_name}'
        )
        response = conn.getresponse()
        body = response.read()
        self.assertEqual(response.status, 200)
        self.assertEqual(body, self.asset_body)
        conn.close()

    def test_public_asset_host_maps_original_path_to_local_store(self):
        conn = http.client.HTTPConnection('127.0.0.1', self.port, timeout=3)
        conn.request(
            'GET',
            f'/zh-android/{self.asset_name}',
            headers={'Host': 'assets.rainbowunicorn7297.com'},
        )
        response = conn.getresponse()
        body = response.read()
        self.assertEqual(response.status, 200)
